...
export const parseXml = (chunk: string) => {
  ...
  const artifactTitleMatch = state.buffer.match(/<boltArtifact[^>]*title="([^"]*)"[^>]*>/);
  if (artifactTitleMatch) {
    console.log("Found Artifact:", artifactTitleMatch);
    state.buffer = state.buffer.replace(artifactTitleMatch[0], "");
  }

  // Extract end artifact
  const endArtifactMatch = state.buffer.match(/<\/boltArtifact>\s*``+/);
  if (endArtifactMatch) {
    state.buffer = state.buffer.replace(endArtifactMatch[0], "");
  }

  // Extract bolt action
  const boltActionMatch = state.buffer.match(
    /<boltAction\s+type\s*="([^"]+)"(?:\s+filePath\s*="([^"]+)")?>/
  );
  if (boltActionMatch) {
    const [, type, filePath] = boltActionMatch;
    console.log("Bolt Action: ", filePath);
    ...
    // Don't replace the buffer yet, wait until we've extracted the content
    // state.buffer = state.buffer.replace(boltActionMatch[0], "");
  }

  // Extract action close
  const actionCloseMatch = state.buffer.match(/<\/boltAction>/);
  if (actionCloseMatch && state.MessageState) {
    const contentBeforeClose = state.buffer.substring(0, actionCloseMatch.index);
    console.log("Content before close", contentBeforeClose);
    ...
    if (currentStepIndex >= 0) {
      step[currentStepIndex].code = state.MessageState.content
        .replace(/<boltAction[^>]*>/g, '') // Remove opening tag
        .replace(/<\/boltAction>/g, ''); // Remove closing tag
    }

    state.MessageState = undefined;
    state.buffer = state.buffer.substring(actionCloseMatch.index + actionCloseMatch[0].length);
  }
